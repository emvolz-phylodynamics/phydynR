...
risk ratio of a male to transmit to a female, and fixed it to $2.0$. This is the 
...
knitr::include_graphics("figures/SN_model_v2.png")
```
...
estimate the spline function for the $gp$ ($gpspline$ in R or $\mu(t)$ in the ODEs). 
And parameters $msmsp0$, $msmsp1$, $msmsp2$, $msmsploc$ are necessary to 
estimate the spline function for the $msm$ risk group ($msmspline$ in R 
or $\lambda(t)$ in the ODEs).
...
For the Senegal data, we used a `maxHeight` = 41. This means that it will go back 
in time in the tree at approximately 1973. This was merely chosen to put the 
...
After setting up the densities and sampler funtions we can now set up our prior 
...
In our paper, we have also extended this analysis by providing a z-matrix. 
First we run a MCMC as above, and after having checking the traces and they are ok, we used that run to provided a z-matrix as explained [here]("https://github.com/florianhartig/BayesianTools/issues/79"). For details on how this was done, you can see our [script](https://github.com/thednainus/senegalHIVmodel/blob/master/analyses/scripts/R-scripts/Models/Subtypes_Combined/Model2/4.mcmc_zMatrix.v2.R) for an example.
...
Here we will load a example of the analysis and check for the estimate parameter
value for one of the parameters we are interested in.
